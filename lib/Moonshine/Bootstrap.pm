...
BEGIN {
    ...
    my @lazy_components = qw/li ul a th td tr p div span b i u dl dt em h1 h2 h3 h4 h5 h6 ol/;
...
sub glyphicon {
	...
	$element_args{class} = sprintf "glyphicon glyphicon-%s", $element_args{class};
	return Moonshine::Element->new(\%element_args);
}
...
sub button_group { 
    ...
    if (my $group_sizing = $element_args{sizing}) {
        $element_args{class} = sprintf '%s btn-group-%s', $element_args{class}, $group_sizing;
    }
...
sub dropdown_ul {
    ...
    if ($headers) {
        for (@{ $headers }) {
            ...
            splice @{ $ul->{children} }, $index - 1, 0, $self->dropdown_header_li($_)
        }
    }
